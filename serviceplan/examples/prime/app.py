...
def run(inputs: dict, tracer: core.Tracer):
    import serviceplan.examples.prime.calculator as calculator
...
async def enter(request: fastapi.Request, inputs: dict):
    return core.Launch(
        request, "serviceplan.examples.prime.app:run", inputs=inputs)
...
async def enter_a(request: fastapi.Request, inputs: dict):
    return core.Launch(
        request, "serviceplan.examples.prime.app:run_async", inputs=inputs)
...
if __name__ == "__main__":
    import uvicorn
    uvicorn.run(
        "serviceplan.examples.prime.app:app", 
        host="0.0.0.0", 
        port=8012, 
        reload=True
    )
